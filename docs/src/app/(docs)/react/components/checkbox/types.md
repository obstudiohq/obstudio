...
[//]: types.ts '<-- Autogenerated By (do not edit the following markdown directly), run: pnpm docs:validate "(docs)/react/components/checkbox"'
...
| value           | `string`                                                                                    | -           | The checkbox's value. Identifies it within a [Checkbox Group](https://obstudio.co/react/components/checkbox-group), falling back to `name` when omitted.&#xA;When submitting a form, a checked box submits `value`; with no `value`, it submits the native "on". |
...
| parent          | `boolean`                                                                                   | `false`     | Whether the checkbox controls a group of child checkboxes. Must be used in a [Checkbox Group](https://obstudio.co/react/components/checkbox-group).                                                                                                              |
