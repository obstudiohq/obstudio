...
<Subtitle>Learn how to make the most of Obstudio's accessibility features and guidelines.</Subtitle>
<Meta
  name="description"
  content="Learn how to make the most of Obstudio's accessibility features and guidelines."
/>

Accessibility is a top priority for Obstudio.
Obstudio components handle many complex accessibility details including ARIA attributes, role attributes, pointer interactions, keyboard navigation, and focus management.
The goal is to provide an accessible user experience out of the box, with intuitive APIs for configuration.

This page highlights some of the key accessibility features of Obstudio, as well as some ways you need to augment the library, to ensure that your application is accessible to everyone.

## Keyboard navigation

Obstudio components adhere to the [WAI-ARIA Authoring Practices](https://www.w3.org/WAI/ARIA/apg/) to provide basic keyboard accessibility out of the box.
...
Obstudio components manage focus automatically following a user interaction.
Additionally, some components provide props like `initialFocus` and `finalFocus`, to configure focus management.

While Obstudio components manage focus, it's the developer's responsibility to visually indicate focus.
...
Obstudio provides components like Form, Input, Field, Fieldset to automatically associate form controls. Additionally, you can use the native HTML `<label>` element to provide context to corresponding inputs.
...
Obstudio components are tested on a broad spectrum of browsers, devices, platforms, screen readers, and environments.

export const metadata = {
  keywords: [
    'Obstudio Accessibility',
    'React Accessibility Guidelines',
    'Keyboard Navigation Obstudio',
    'Focus Management Tips',
    'Color Contrast Guidance',
    'Accessible Labels Obstudio',
