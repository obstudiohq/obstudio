...
- Event handlers are merged and executed right-to-left (rightmost first):
  ...
  - For React synthetic events, Obstudio adds `event.preventObstudioHandler()`. Calling it prevents Obstudio's internal logic from running.
...
### Preventing Obstudio's default behavior

import { DemoPreventObstudioHandler } from './demos/prevent-obstudio-handler';

When using the function form of the `render` prop, props are not merged automatically.
You can use `mergeProps` to combine Obstudio's props with your own, and call `preventObstudioHandler()` to stop Obstudio's internal logic from running:

<DemoPreventObstudioHandler />
