...
<Subtitle>
  Configures CSP-related behavior for inline tags rendered by Obstudio components.
</Subtitle>
<Meta
  name="description"
  content="A CSP provider component that applies a nonce to inline <style> and <script> tags rendered by Obstudio components, and can disable inline <style> elements."
/>
...
This will ensure that all inline `<style>` and `<script>` tags rendered by Obstudio components include the correct nonce attribute, allowing them to function under your CSP.
...
```html
<style>
  .obstudio-disable-scrollbar {
    scrollbar-width: none;
  }
  .obstudio-disable-scrollbar::-webkit-scrollbar {
    display: none;
  }
</style>
```
...
3. Manually unset inline styles and specify them in your CSS instead. Any component can have its inline styles unset, such as `<ScrollArea.Viewport style={{ overflow: undefined  }}>`. Note that you'll need to ensure you vet upgrades for any new inline styles added by Obstudio components.
