...
<Subtitle>A guide to customizing the behavior of Obstudio components.</Subtitle>
<Meta name="description" content="A guide to customizing the behavior of Obstudio components." />

## Obstudio events

Change events such as `onOpenChange`, `onValueChange`, and `onPressedChange` are custom to Obstudio.
They can be emitted by various different DOM events, effects, or even during rendering.

```js title="Obstudio event signatures"
onOpenChange: (open, eventDetails) => void
onValueChange: (value, eventDetails) => void
onPressedChange: (pressed, eventDetails) => void
```

The `eventDetails` property is passed as a second argument to Obstudio event handlers.
...
interface ObstudioChangeEventDetails {
  reason: string;
  event: Event;
  cancel: () => void;
...
- `allowPropagation` allows the DOM event to propagate in cases where Obstudio stops the propagation.
...
## Preventing Obstudio from handling a React event

To prevent Obstudio from handling a React event like `onClick`, you can use the `preventObstudioHandler()` method on the event object:

```tsx title="Prevent Obstudio's default behavior"
<NumberField.Input
  onPaste={(event) => {
...
export const metadata = {
  keywords: [
    'Obstudio Customization',
    'React eventDetails',
    'Control Obstudio State',
    'Cancel Obstudio Events',
