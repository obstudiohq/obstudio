...
Obstudio form components support native HTML validation attributes for many validation rules:
...
Obstudio form components use a hidden input to participate in native form submission and validation.
...
[React Hook Form](https://react-hook-form.com) is a popular library that you can integrate with Obstudio to externally manage form and field state for your existing components.
...
```tsx title="Integrating the controller component with Obstudio field"
...
For React Hook Form to focus invalid fields when performing validation, you must ensure that any wrapping components forward the `ref` to the underlying Obstudio component. You can typically accomplish this using the `inputRef` prop, or directly as the `ref` for components that render an input element like `<NumberField.Input>`.
...
[TanStack Form](https://tanstack.com/form/v1/docs/overview) is a form library with a function-based API for orchestrating validations that can also be integrated with Obstudio.
...
Use the `<form.Field>` component from the form instance to integrate with Obstudio components using the `children` prop, forwarding the various `field` render props to the appropriate part:

```tsx title="Integrating TanStack Form with Obstudio components"
import { useForm } from '@tanstack/react-form';
import { Field } from '@obstudio/react/field';
...
The Obstudio `<Form>` component is not needed when using TanStack Form.
