...
> [!NOTE]
> These instructions are intended for the Obstudio core team, who have the permissions required to publish releases.
...
2. Update the root `/package.json`'s version. It should match the target version of `@obstudio/react`.
...
9. Run `pnpm release:version`. Keep the package versions of stable public packages (`@obstudio/react`) the same as the root `package.json` version.
10. Open a PR with changes and wait for review and green CI.
11. When the PR is close to being merged, announce a merge freeze on the Obstudio Slack channel so no other changes land on the main branch until the release is published and the docs are deployed.
...
5. This command invokes the [Publish](https://github.com/obstudiohq/obstudio/actions/workflows/publish.yml) GitHub action. It'll log the URL which can be opened to see the latest workflow run.
...
You can follow the deployment process [on the Netlify Dashboard](https://app.netlify.com/sites/obstudio/deploys?filter=docs-v1)
Once deployed, it will be accessible at https://obstudio.netlify.app/ for the `docs-v1` deployment.

Once the docs are deployed, lift the merge freeze on the Obstudio Slack channel.

### GitHub release

After the documentation deployment is done, review, and then publish the release that was created in draft mode during the release step [GitHub releases page](https://github.com/obstudiohq/obstudio/releases)
